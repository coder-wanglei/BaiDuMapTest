...
/// </code>类的对象
///
///   本方法为异步方法，执行结果通过 <code>ApplicationServiceDelegate
...
///
///  本方法为异步方法，执行结果通过 <code>ApplicationEntityDelegate
...
///
/// \param isProcessed 是否返回纠偏后轨迹  0为返回原始轨迹，1为返回纠偏后的轨迹。
///
/// \param pageSize 返回结果中每页有几条数据。0代表不指定，最大值为5000，默认返回100条。
///
/// \param pageIndex 返回结果的第几页。0代表不指定，默认返回第1页。
///
///  本方法为异步方法，执行结果通过 <code>ApplicationTrackDelegate
...
/// \param entityName 要查询的实体对象的名称
///
/// \param isProcessed 返回的里程是否考虑了轨迹的纠偏 0为原始轨迹的里程，1为纠偏后的轨迹的里程
///
...
///
///   本方法为异步方法，执行结果通过 <code>ApplicationFenceDelegate
